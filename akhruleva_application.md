...
Заказчик часто использует своё транспортное средство для передвижения по городу, также иногда транспортное средство используется для передвижения на дальние расстояния. Кроме того, заказчик ведёт учёт различных показателей: пробег, топливо, расходы на запчасти/материалы. На данный момент учёт происходит в word/excel, что не всегда является удобным, поэтому заказчику необходимо удобное мобильное приложение для ведения учёта в нём. 
